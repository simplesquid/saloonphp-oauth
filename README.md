...
[<img src="https://github-ads.s3.eu-central-1.amazonaws.com/saloonphp-oauth.jpg?t=1" width="419px" />](https://spatie.be/github-ad-click/saloonphp-oauth)
...
```bash
php artisan vendor:publish --tag="saloonphp-oauth-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="saloonphp-oauth-config"
```
...
```bash
php artisan vendor:publish --tag="saloonphp-oauth-views"
```

## Usage

```php
$saloonOAuth = new SimpleSquid\SaloonOAuth();
echo $saloonOAuth->echoPhrase('Hello, SimpleSquid!');
```
